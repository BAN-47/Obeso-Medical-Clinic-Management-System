    ...
    try {
        ...
        if ($action === 'mark_done') {
            ...
            echo json_encode([
                'success' => true, 
                'queue_id' => $queueId,
                'patient_name' => $queueInfo['full_name'] ?? null,
        ...
        if ($action === 'fetch_patient_details') {
            ...
            $cstmt = $db->prepare("SELECT * FROM checkups WHERE patient_id = ? ORDER BY created_at DESC LIMIT 1");
            $cstmt->execute([$patientId]);
            $lastCheckup = $cstmt->fetch(PDO::FETCH_ASSOC);

            echo json_encode(['success' => true, 'patient' => $patient, 'last_checkup' => $lastCheckup]);
            exit();
        }
...
<!-- Patient details modal (AJAX populated) -->
<div class="modal-overlay" id="patientDetailsModal">
    <div class="modal-box" id="patientDetailsBox">
        <h5 id="pd-name">Patient Details</h5>
        <p id="pd-sub">Loading…</p>
        <div id="pd-body" style="text-align:left; margin-top:8px;"></div>
        <div class="modal-actions" style="margin-top:14px;">
            <a id="pd-open-record" class="btn btn-outline-primary" href="#">Open Medical Record</a>
            <button class="btn btn-secondary" onclick="closePatientDetails()">Close</button>
        </div>
    </div>
</div>
...
function markDone(id) { 
    postAction('mark_done', { queue_id: id }).then(d => {
        fetchQueue();
    }).catch(() => fetchQueue());
}
...
/* Patient details modal control */
function closePatientDetails() { document.getElementById('patientDetailsModal').classList.remove('show'); }
function showPatientDetails(patientId) {
    ...
    document.getElementById('pd-body').innerHTML = '<div class="text-center"><i class="fa-solid fa-spinner fa-spin"></i> Loading…</div>';
    document.getElementById('pd-open-record').setAttribute('href', 'doctor_medical_records_management.php?patient_id=' + encodeURIComponent(patientId));
    document.getElementById('patientDetailsModal').classList.add('show');

    postAction('fetch_patient_details', { patient_id: patientId }).then(d => {
        if (!d.success) {
            document.getElementById('pd-name').textContent = 'Error';
            document.getElementById('pd-body').textContent = d.error || 'Unable to fetch patient details.';
            return;
        }
        const p = d.patient || {};
        const c = d.last_checkup || null;
        document.getElementById('pd-name').textContent = p.full_name || 'Patient Details';
        document.getElementById('pd-sub').textContent = (p.age ? p.age + ' yrs' : '') + (p.sex ? (' · ' + p.sex) : '');

        let body = '<dl class="row">';
        body += '<dt class="col-4">Contact</dt><dd class="col-8">' + (p.contact_no || '—') + '</dd>';
        body += '<dt class="col-4">Address</dt><dd class="col-8">' + (p.address || '—') + '</dd>';
        if (c) {
            body += '<dt class="col-4">Last Checkup</dt><dd class="col-8">' + (c.notes ? escHtml(c.notes) : '—') + '</dd>';
            body += '<dt class="col-4">BP / Temp</dt><dd class="col-8">' + (c.blood_pressure || '—') + ' / ' + (c.temperature || '—') + '</dd>';
        }
        body += '</dl>';
        document.getElementById('pd-body').innerHTML = body;
    }).catch(err => {
        document.getElementById('pd-name').textContent = 'Error';
        document.getElementById('pd-body').textContent = String(err);
    });
}
...
function renderNowServing(inprog) {
    ...
    box.innerHTML = `<div class="now-serving-banner">
        ...
        <div class="flex-grow-1">
            <div class="ns-name" style="cursor:pointer" onclick="showPatientDetails(${inprog.patient_id})">${escHtml(inprog.full_name)}</div>
            <div class="ns-meta">
...
function renderTable(rows) {
    ...
    tbody.innerHTML = rows.map(r => {
        ...
        return `<tr class="${isDone ? 'row-done' : ''}">
            ...
            <td>
                <div class="fw-semibold" style="font-size:.875rem; cursor:pointer;" onclick="showPatientDetails(${r.patient_id})">${escHtml(r.full_name)}</div>
